// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.36.6
// 	protoc        v3.21.12
// source: common/info.proto

package common

import (
	_ "github.com/grpc-ecosystem/grpc-gateway/v2/protoc-gen-openapiv2/options"
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	reflect "reflect"
	unsafe "unsafe"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

var File_common_info_proto protoreflect.FileDescriptor

const file_common_info_proto_rawDesc = "" +
	"\n" +
	"\x11common/info.proto\x12\x06common\x1a.protoc-gen-openapiv2/options/annotations.protoB\xd0\x02\x92A\x8c\x02\x12y\n" +
	"\x10segmentation API\x12-Methods to interact with segmentation backend\"1\x12/https://github.com/MelnikovNA/segmentationproto2\x030.0*\x02\x02\x012\x10application/json:\x10application/jsonZY\n" +
	"W\n" +
	"\x06bearer\x12M\b\x02\x128Authentication token, prefixed by Bearer: Bearer <token>\x1a\rAuthorization \x02b\f\n" +
	"\n" +
	"\n" +
	"\x06bearer\x12\x00Z>ithub.com/MelnikovNA/User-segmentation/proto/codegen/go/commonb\x06proto3"

var file_common_info_proto_goTypes = []any{}
var file_common_info_proto_depIdxs = []int32{
	0, // [0:0] is the sub-list for method output_type
	0, // [0:0] is the sub-list for method input_type
	0, // [0:0] is the sub-list for extension type_name
	0, // [0:0] is the sub-list for extension extendee
	0, // [0:0] is the sub-list for field type_name
}

func init() { file_common_info_proto_init() }
func file_common_info_proto_init() {
	if File_common_info_proto != nil {
		return
	}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: unsafe.Slice(unsafe.StringData(file_common_info_proto_rawDesc), len(file_common_info_proto_rawDesc)),
			NumEnums:      0,
			NumMessages:   0,
			NumExtensions: 0,
			NumServices:   0,
		},
		GoTypes:           file_common_info_proto_goTypes,
		DependencyIndexes: file_common_info_proto_depIdxs,
	}.Build()
	File_common_info_proto = out.File
	file_common_info_proto_goTypes = nil
	file_common_info_proto_depIdxs = nil
}
